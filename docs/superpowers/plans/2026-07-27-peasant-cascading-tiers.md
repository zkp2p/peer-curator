...
>
> **Status: executed, then split across two branches.** All nine tasks were implemented and
> verified as written. The result was afterwards divided into `feat/cascading-tiers` (six
> cascading groups) and `feat/peasant-tier` (the fourth tier, eight groups). The task breakdown
> below reflects the original combined implementation and is kept as the record of how it was
> built — it is not a plan for either branch taken alone.
